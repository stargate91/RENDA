...
def map_resolution(width: int, height: int) -> str:
    """FFprobe width/height → scene-standard felbontás."""
    if not width or not height:
        return ""
    
    # Magasság alapján (portrait videóknál a kisebbik)
...
def map_video_codec(codec_name: str) -> str:
    """FFprobe codec_name → scene-standard videó kodek."""
    if not codec_name:
        return ""
...
# DTS profilok (a profile mező alapján finomítjuk)
_DTS_PROFILE_MAP = {
    "DTS-HD MA": "DTS-HD.MA",
...
def map_audio_codec(codec_name: str, profile: str = None) -> str:
    """FFprobe codec_name + profile → scene-standard audió kodek."""
    if not codec_name:
        return ""
    
    key = codec_name.lower()
    
    # DTS speciális profilok
    if key == "dts" and profile:
        return _DTS_PROFILE_MAP.get(profile, "DTS")
    
    # TrueHD + Atmos detektálás (a side_data-ból derül ki)
    # Ezt a hívó oldalon kell kezelni, mert az Atmos flag a stream metaadatban van
...
def map_audio_channels(channels: int) -> str:
    """FFprobe channels szám → scene-standard csatornaszám."""
    if not channels:
        return ""
...
def map_hdr(color_transfer: str = None, color_primaries: str = None, 
    ...
    """FFprobe szín/HDR adatok → scene-standard HDR típus."""
    if has_dovi:
        return "DV"
...
def map_bit_depth(bits_per_raw_sample: str = None, pix_fmt: str = None) -> str:
    """FFprobe bit depth → scene-standard formátum."""
